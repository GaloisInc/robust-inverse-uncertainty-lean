...
picture for the Boolean cube. It adds the following conclusions:
\begin{itemize}
  \item It proves the existence of a subspace $H$ and shifts $a,b$ for which
  $S$ and $T$ have explicit symmetric-difference bounds from $a+H$ and
  $b+H^\perp$.
  \item It controls the full complex vector in squared $\ell_2$ distance,
  including modulation and global phase.
  \item It gives concrete constants and a complete Lean formalization of the
  geometric and vector conclusions.
...
Relative to Miller's coefficient-level result, the added content is quantitative
geometric and vector rigidity in the Boolean setting. The theorem proves the
existence of paired cosets with quantitative symmetric-difference bounds
under a concrete two-basis noise model. It also shows that near saturation
identifies the quantum state up to the expected translation, modulation, and
global phase. Formalization is useful here because the argument combines
Fourier normalization, additive combinatorics, projector algebra, and many
explicit constants.
